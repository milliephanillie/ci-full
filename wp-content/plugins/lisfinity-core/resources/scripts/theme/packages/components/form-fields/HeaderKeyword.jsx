...
const HeaderKeyword = (props) => {
  ...
  const showMenuHandler = () => {
    if (window.innerWidth < 1030) {
      if ((lc_data.is_search || lc_data.is_business)) {
  ...
  useEffect(() => {
    if (data.searchData.showFilters) {
      setMenuActivated(false);
    } else {
      setMenuActivated(true);
    }
  ...
  return (
    <div className={`keyword--wrapper`} ref={el}>
      {!lc_data.is_front && !lc_data.is_search && !lc_data.is_business && !loading && menuActivated &&
      <div className="filters flex-center absolute"
           style={{
             top: 30,
             right,
           }}
      >
        <button type="button"
          ...
          <ReactSVG
            ...
            className={`relative -top-2 w-22 h-22 fill-white`}
          />
          }
          {showKeyword &&
          <ReactSVG
      ...
      {!loading && (showKeyword || !menuActivated) &&
      <div key={0} className="keyword-div flex items-center w-full">
        <form key={1} action={lc_data.page_search} className={`flex-center mx-auto w-full`}>
          <Keyword keywordValueHandle={(value) => keywordValueHandle(value)} fields={fields}
                   searchValues={settings}
                   displayLabel = {keywordSettings.display_label}
                   type="header"/>
          <button
            type="submit"
            className={`btn btn-lg -ml-3 lisfinity-order-10 px-10 h-40 bg-blue-700 hover:bg-blue-800 w-auto bg:w-auto rounded-l-none z-10`}
            onClick={(e) => {
